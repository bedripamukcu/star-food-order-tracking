  ...
  useEffect(() => {
    const getOrders = async () => {
      try {
        ...
        const parcelReadyOrders = await fs.get("parcelready");
        setOrders(parcelReadyOrders);
      } catch (err) {
        console.log(err);
      }
    };
    ...
    <>
      ...
      >
        <div className="d-flex align-items-center">
          ...
          <div className="custom-header ">Parcel Ready </div>
        </div>
      </div>
      <br />
      <div
      ...
      <div style={{ margin: "10px 70px 10px -20px" }}>
        {orders &&
          orders.map((o) => {
            return <OrderSection order={o} status="delivered" />;
          })}
      </div>
    </>
  );
