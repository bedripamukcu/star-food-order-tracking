  ...
  useEffect(() => {
    const getOrders = async () => {
      try {
        ...
        const deliveredOrders = await fs.get("delivered");
    ...
    <>
      ...
      >
        <div className="d-flex align-items-center">
          ...
          <div className="custom-header ">Delivered </div>
        </div>
      </div>
      <br />
      <div
      ...
      <div style={{ margin: "10px 70px 10px -20px" }}>
        {orders &&
          orders.map((o) => {
            return <OrderSection order={o} status="completed" />;
          })}
      </div>
    </>
  );
