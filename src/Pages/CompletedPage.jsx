  ...
  useEffect(() => {
    const getOrders = async () => {
      try {
        ...
        const completedOrders = await fs.get("completed");
    ...
    <>
      ...
      >
        <div className="d-flex align-items-center">
          ...
          <div className="custom-header ">Completed </div>
        </div>
      </div>
      <br />
      <div
      ...
      <div style={{ margin: "10px 70px 10px -20px" }}>
        {orders &&
          orders.map((o) => {
            return <OrderSection order={o} status={null} />;
          })}
      </div>
    </>
  );
